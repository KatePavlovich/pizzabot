...
export const App = () => {
    ...
    const boardSize = getBoardSize(command);
		...
		if (!coordinates) return setError('Please check coordinates');
		if (checkCoordinates(coordinates)) return setError('Please check coordinates. Miss some values');
		if (checkCoordinateValues(coordinates, boardSize))
			return setError('Seems like some coordinates are far from available delivery zone');
		if (coordinates) {
	...
	return (
		<div className={styles.content}>
			<form className={styles.form} onSubmit={handleSubmit}>
				...
				<button type="submit" disabled={!!steps.length} className={styles.button}>
					Start delivery
				</button>
				<p>example: 5x5 (0, 0) (1, 3) (4, 4) (4, 2) (4, 2) (0, 1) (3, 2) (2, 3) (4, 1)</p>
			</form>
			<div className={styles.error}>{error}</div>
			{board && !error && <Board board={board} />}
			{!!steps.length && (
				<div className={styles.steps}>
					{steps.map((step, i) => <div key={i}>{i < countSteps ? step : ''}</div>)}
				</div>
			)}
			{board &&
